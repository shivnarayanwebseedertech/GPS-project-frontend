        ...
        <>
            ...
            <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200">
                <div className="flex items-center justify-between p-4">
                    <div className="h-8"><img src={logo} className='h-full w-full object-contain object-center' /></div>
                    <button
            ...
            <nav className="hidden lg:flex flex-col fixed top-0 left-0 h-screen w-64 bg-white border-r border-gray-200 shadow-sm z-30">
                <div className="p-4 space-y-4 border-b border-gray-100">
                    ...
                    <h1 className='capitalize text-gray-600 italic text-center text-sm'>Welcome {user?.name === "" ? "admin" : user?.name}!</h1>
                </div>
                ...
                <div className="flex-grow p-4 overflow-y-auto">
                            ...
                            return (
                                <li key={mod}>
                                    <NavLink
                                        ...
                                        className={({ isActive }) =>
                                            `group flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 ${isActive
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-200">
                        <h3 className="px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Quick Actions</h3>
                        <ul className="space-y-2">
            ...
            {/* Mobile Sidebar */}
            <nav
                className={`sidebar-container lg:hidden fixed top-0 left-0 z-50 h-full w-72 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                <div className="p-4 border-b border-gray-100">
                    ...
                    <h1 className='capitalize text-gray-600 italic text-center text-xs mt-2'>Welcome {user?.name === "" ? "admin" : user?.name}!</h1>
                </div>
                ...
                <div className="flex-grow p-3 overflow-y-auto">
                            ...
                            return (
                                <li key={mod}>
                                    <NavLink
                                        ...
                                        onClick={closeMobileMenu}
                                        className={({ isActive }) =>
                                            `group flex items-center gap-2 px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${isActive
                                                ? 'bg-indigo-50 text-indigo-700 border-r-2 border-indigo-700'
                                                : 'text-gray-700 hover:bg-gray-50 hover:text-indigo-600'
                                            }`
                                        }
                                    >
                                        {navigationIcons[mod]}
                                        <span className="truncate text-sm">{moduleMap[mod].label}</span>
                                    </NavLink>
                                </li>
                            );
                        })}
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-200">
                        <h3 className="px-3 text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Quick Actions</h3>
                        <ul className="space-y-1">
                            {quickActions.map(({ label, path, icon }) => (
                                <li key={label}>
                                    <button
