    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <button
                    onClick={openPanel}
                    className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-lg font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center gap-2 transform hover:-translate-y-0.5"
                >
            ...
            {showPanel && (
                <>
                    {/* Backdrop */}
                    <div
                        className={`fixed h-screen inset-0 bg-black/50 backdrop-blur-sm z-40 transition-opacity duration-400 ${panelState === 'closing' ? 'opacity-0' : 'opacity-100'
                            }`}
                        onClick={closePanel}
                    />

                    {/* Modal Panel */}
                    <div className={`fixed h-screen inset-0 z-50 flex items-center justify-center p-4 transition-all duration-400 ease-out ${panelState === 'opening' || panelState === 'closing'
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-sm">
                                                {editId ? 'Update user information and permissions' : 'Add a new user to your organization'}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                            ...
                            <div className="flex-1 overflow-y-auto p-8">
                                <form onSubmit={handleSubmit} className="space-y-8">
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-6">
                                            {Object.entries(moduleCategories).map(([category, modules], categoryIndex) => (
                                                <div key={category} className="space-y-3">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                                        {modules.map((module, moduleIndex) => (
                                                            <label
                                                                key={module}
                                                                className={`flex items-start p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 hover:shadow-md ${form.modules.includes(module)
                                                                    ? 'border-indigo-200 bg-indigo-50'
                                                                    : 'border-gray-200 hover:border-gray-300'
                                                                    }`}
                                                                style={{
                                                                    animationDelay: `${(categoryIndex * 100) + (moduleIndex * 50)}ms`,
                                                                    animation: panelState === 'open' ? 'slideInUp 0.3s ease-out forwards' : undefined
                                                                }}
                                                            >
                                                                <input
                                                                    type="checkbox"
                                        ...
                                        {form.modules.length > 0 && (
                                            <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
                                                <h5 className="text-sm font-medium text-indigo-900 mb-2">Selected Permissions ({form.modules.length})</h5>
                                    ...
                                    <tr key={u._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-sm text-gray-900">
                                                {u.role === 'admin' ? (
                                                    <span className="text-green-600 font-medium">All Modules</span>
                                                ) : (
                                                    <div className="flex flex-wrap gap-1">
                                                        {(u.modules || []).slice(0, 3).map(mod => (
                                                            <span key={mod} className="inline-block bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs">
                                                                {mod.replace('-', ' ')}
                                                            </span>
                                                        ))}
                                        ...
                                        <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                                            <div className="flex flex-col items-center gap-2">
                                                <svg className="w-12 h-12 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5 0a4 4 0 11-8-1.5 4 4 0 018 1.5z" />
                                                </svg>
                                                No users found
                                            </div>
                                        </td>
